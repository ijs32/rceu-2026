...
class Plotter():
    def __init__(self, surrogate: Surrogate | MFSurrogate):
    ...
    def plot_check_inputs(self, res = 100):
        ...
        if self.surrogate.dim == 1:
            ...
            Nside = res
            x = np.linspace(self.surrogate.xmin[0], self.surrogate.xmax[0], Nside)
            ...
            plt.xlim((self.surrogate.xmin[0], self.surrogate.xmax[0]))

        elif self.surrogate.dim == 2:
            Nside = res
            x0 = np.linspace(self.surrogate.xmin[0], self.surrogate.xmax[0], Nside)
            y0 = np.linspace(self.surrogate.xmin[1], self.surrogate.xmax[1], Nside)
    ...
    def plot_check_model(self, res = 100):
        ...
        if self.surrogate.dim == 1:
            ...
            if isinstance(self.surrogate, MFSurrogate):
                ...
                plt.scatter(
                    self.surrogate.x, self.surrogate.y,
                    **scatter_config1,
                )
            ...
            Z = self.surrogate.evaluate(matr)
            ...
            if isinstance(self.surrogate, MFSurrogate):
                ax2.scatter(self.surrogate.x_c[:,0], self.surrogate.x_c[:,1], color='black', marker='^', s=7.5, alpha=1.0)
                ax2.scatter(self.surrogate.x_e[:,0], self.surrogate.x_e[:,1], color='red', marker='^', s=7.5, alpha=1.0)

            else:
                ax2.scatter(self.surrogate.x[:,0], self.surrogate.x[:,1], color='black', marker='^', s=7.5, alpha=1.0)
